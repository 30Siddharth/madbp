...
class MovingHorizonTrajectoryExecutor:
    def __init__(self, scenario_name=None, num_envs=1, max_steps=40, n_agents=4, device='cuda',
                 test_start_positions=None, test_goal_positions=None, horizon_size=8, goal_tolerance=0.05):
        """
        ...
        self.goal_tolerance = goal_tolerance  # Distance threshold for success
        ...
        self.env = make_env(
            ...
            goal_tolerance=self.goal_tolerance  # Add goal tolerance parameter
        )
    ...
    def compute_control_forces(self, current_states, target_positions, target_velocities=None):
        ...
        current_pos = torch.stack([agent.state.pos for agent in self.env.agents], dim=1)
        current_vel = torch.stack([agent.state.vel for agent in self.env.agents], dim=1)
        
        # Check which agents have reached their goals
        goal_positions = torch.stack([agent.goal.state.pos for agent in self.env.agents], dim=1)
        distances_to_goal = torch.norm(current_pos - goal_positions, dim=-1)
        agents_at_goal = distances_to_goal < self.goal_tolerance
        ...
        # Zero out forces for agents that have reached their goals
        forces = torch.where(
            agents_at_goal.unsqueeze(-1).expand_as(forces),
    ...
    def execute_moving_horizon_trajectory(self, model, frames, starts, goals, n_agents,
        ...
        # Track which agents have reached their goals
        n_active = n_agents.item()
        agents_completed = torch.zeros(n_active, dtype=torch.bool, device=self.device)
        ...
        with torch.no_grad():
            ...
            horizon_prediction = model.sample_with_constraints(
                ...
                steps=50,  # Reduced steps for real-time performance
                max_step_size=0.05
        ...
        smoothed_segment = self.smooth_trajectory_segment(
            horizon_segment, method='velocity_smoothing', strength=0.2
        )
        ...
        for t in range(min(current_horizon_length - 1, self.max_steps - len(executed_positions) - 1)):
            ...
            target_pos = smoothed_segment[t + 1, :n_active].unsqueeze(0).to(self.device)
            ...
            # Compute control forces and check goal status
            forces, agents_at_goal = self.compute_control_forces(None, target_pos, target_vel)

            # Update completion status
            agents_completed |= agents_at_goal[0]

            # Create actions (zero for completed agents)
            batch_actions = torch.zeros(self.n_agents, 2, device=self.device)
            for agent_idx in range(self.n_agents):
                if agent_idx < n_active and not agents_completed[agent_idx]:
            ...
            # Check if done or all agents completed
            if dones.any():
                print(f"Episode finished at step {len(executed_positions)}")
                break

            # Check if all agents completed
            if agents_completed.all():
                print(f"All agents reached their goals at step {len(executed_positions)}")
                break

        # Break if all agents completed or episode done
        # if agents_completed.all() or dones.any():
        #     break
        ...
        return {
            ...
            'agents_completed': agents_completed.cpu()
        }
    ...
    def evaluate_moving_horizon_performance(self, results):
        ...
        # Check final distances to goals
        final_positions = executed[-1]  # [n_agents, 2]
        goal_positions = self.test_goal_positions[:final_positions.shape[0]]  # Match agent count

        final_distances = torch.norm(
            torch.tensor(final_positions) - torch.tensor(goal_positions), 
            dim=-1
        )

        # Success based on goal tolerance
        ...
        if len(results['planned_segments']) > 0:
            ...
            executed_trimmed = executed[:min_length]
            planned_trimmed = planned_full[:min_length]

            # Compute metrics
            position_errors = torch.norm(executed_trimmed - planned_trimmed, dim=-1)
        ...
        final_position_error = final_distances.mean().item()
        ...
        metrics = {
            ...
            'final_distances_to_goals': final_distances.tolist(),
            'agents_successful': agents_successful.tolist(),
            'total_reward': total_reward,
            'success_rate': success_rate,
            'avg_generation_time': avg_generation_time,
            'total_horizons': total_horizons,
            'real_time_capable': avg_generation_time < 0.4
        }
...
def execute_moving_horizon_diffusion_trajectory_in_vmas(scenario_name, model_path=None, device='cuda', goal_tolerance=0.05):
    ...
    if model_path is None:
        model_path = f"boundary_constrained_madp_{scenario_name}.pth"  # Match v2_2 naming
    ...
    executor = MovingHorizonTrajectoryExecutor(
        ...
        goal_tolerance=goal_tolerance  # Add goal tolerance parameter
    )
    ...
    print(f"Agents Successful: {metrics.get('agents_successful', [])}")
...
if __name__ == "__main__":
    ...
    # Execute moving horizon trajectory with custom goal tolerance
    results, metrics = execute_moving_horizon_diffusion_trajectory_in_vmas(
        "navigation_v3_test2",
        model_path="boundary_constrained_madp_navigation_v3.pth",  # v2_2 model
        device=device,
        goal_tolerance=0.2  # Adjust this value as needed
    )
