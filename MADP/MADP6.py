...
class MultiAgentDiffusionPolicy(nn.Module):
    ...
    # def component_weighted_loss(pred, target, pos_weight=1.0, velo_weight=0.5, action_weight=0.2):
    #     """
    #     pred/target shape: [B, T, Agents, D] where D = pos(2) + velo(2) + action(2)
    #     """
    #     pdb.set_trace()
    ...
    #         raise ValueError(f"Inputs must be 4D tensors. Got pred: {pred.ndim}D, target: {target.ndim}D")
    ...
    #     target_pos = target[..., :2]
    #     target_velo = target[..., 2:4]
    #     target_action = target[..., 4:]
    ...
    #     return (pos_weight * pos_loss 
    #             + velo_weight * velo_loss 
