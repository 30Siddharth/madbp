...
from MADP6_states import MultiAgentDiffusionPolicy
...
def train_diffusion_model(model, dataloader, test_dataloader=None, optimizer=None, 
    ...
    for epoch in range(num_epochs):
        ...
        writer.add_scalar('Loss/MADP3_states/epoch', avg_train_loss, epoch)
...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        states_actions = states_actions[:,:,:2]
...
if __name__=='__main__':
    ...
    model = MultiAgentDiffusionPolicy(
        ...
        horizon=15,  # Set to match your trajectory length
        hidden_dim=32,
    ...
    if not train_config['param']['train']:
        model.load_state_dict(torch.load("diffusion_policy.pth"))
        model.eval()
        ...
        for step, traj in intermediate_trajectories.items():
            ...
            for agent_id in range(test_n_agents):
                ...
                pred_xy = pos_pred[:, agent_id]
                plt.scatter(pred_xy[:, 0], pred_xy[:, 1], color=colors[agent_id], alpha=step/15,
