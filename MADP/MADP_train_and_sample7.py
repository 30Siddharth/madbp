...
from MADP7 import MultiAgentDiffusionPolicy
# from MADP_agent_diffuse import MultiAgentDiffusionPolicy



def plot_and_save_denoise(intermediate_trajectories, test_states_actions):
    '''
    Plot the denoising steps in sampling. Save a GIF.
    '''
    
    step_keys = list(intermediate_trajectories.keys())
    for i  in range(len(intermediate_trajectories)):
        ...
        k = step_keys[i]
        pos_pred = intermediate_trajectories[k]
        for agent_id in range(num_agents):
            pred_xy = pos_pred[:, agent_id]
            plt.scatter(pred_xy[:, 0], pred_xy[:, 1], color=colors[agent_id], 
                    label=f'Agent {agent_id} Pred')

        print(f'Saved plot for denoised trajectory at k = {k}')
        plt.title(f'Denoised trajectory at k = {k}')
        plt.legend()
        plt.grid(True)
        plt.xlim([-1,1])
        plt.ylim([-1,1])
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"diffusion_steps/Denoised_trajectory_k_{k}.png")
        plt.close()
    ...
    try:
        ...
        filenames = sorted(glob.glob("diffusion_steps/Denoised_trajectory_k_*.png"))
        
        # Add ground truth at the beginning
        # filenames = ["diffusion_steps/ground_truth.png"] + filenames
        
        # Create GIF
        with imageio.get_writer("diffusion_steps/diffusion_process.gif", mode="I", duration=0.5) as writer:
...
def train_diffusion_model(model, dataloader, test_dataloader=None, optimizer=None, 
    ...
    for epoch in range(num_epochs):
        ...
        for  batch_idx, batch in enumerate(dataloader):
            # horizon_curr = max(int(5 + 100*(epoch/num_epochs)), horizon)
            # TODO: Update this to introduce curriculum in the horizon dimension.
            ...
            if (epoch + 1) % 1000 == 0:
                ...
                with torch.no_grad():
                    ...
                    # test_batch = batch
                    test_states_actions = test_batch['states_actions'][0:1][:2*horizon:2].to(device)
                    test_context = tuple(c[0:1].to(device) for c in test_batch['context'])
                    ...
                    # test_states_actions = test_batch['states_actions'][:horizon].to(device)
                    # print("test_states_actions: ", test_states_actions.shape)
                    # print("test_context: ", test_context[0].shape)
                    # pdb.set_trace()
                    # test_context = test_context[0].squeeze(0)

                    generated_trajectory, intermediate_trajectories = model.sample_trajectory(test_context, num_agents, save_steps=True)
                    

                    plot_and_save_denoise(intermediate_trajectories, test_states_actions)
                    ...
                    ax1.set_title(f'Pose Trajectories at Epoch {best_epoch+1} and loss {best_test_loss}')
                    ax1.legend()
                    ax1.grid(True)
                    ax1.set_xlim([-1,1])
                    ax1.set_ylim([-1,1])
...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pdb.set_trace()
        states_actions = torch.tensor(self.h5_file['trajectories'][episode_name][:2*self.horizon:2], dtype=torch.float32)
...
if __name__=='__main__':
    ...
    train_dataset = TrajectoryDataset(h5_path=h5_path, split='train', horizon=train_horizon)
    test_dataset = TrajectoryDataset(h5_path=h5_path, split='test', horizon=train_horizon)
    ...
    model = MultiAgentDiffusionPolicy(
        ...
        horizon=train_horizon,  # Set to match your trajectory length
        hidden_dim=32,
