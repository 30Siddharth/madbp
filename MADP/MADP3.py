...
Date: 06.04.2025
Note: Written with the help of LLM Tools - modified for use case. Inspired from Video Diffusion code by Lucid and Diffusion Policy. 
[*] Video Diffusion: https://github.com/lucidrains/video-diffusion-pytorch
[*] Stanford Diffusion Policy: https://github.com/real-stanford/diffusion_policy/tree/main
'''
...
class MultiAgentDiffusionPolicy(nn.Module):
    def __init__(
        ...
        image_channels: int = 3, # Number of channels in the observation images
        ...
        super().__init__()
        self.state_dim = state_dim  # State here refers to the state-action pair [s,a]
        ...
        self.image_encoder = nn.Sequential(
            nn.Conv2d(image_channels, 64, kernel_size=5, stride=2, padding=1),
            nn.ReLU(),
    ...
    def sample_trajectory(self, context, n_agents, shape=None):
        ...
        '''
        TODO: Improve this to handle cases when conext for training is different from conext in evaluation. 
        '''
        
        # if self.training :
        #     # Process full context with past trajectories
        #     context_features = self.process_context(context)
        # else:
        #     # Process context with only current observation
        #     context_features = self.process_current_step_context(context)
...
class TrajectoryUNet(nn.Module):
    ...
    def forward(self, x, time_emb, context_features, n_agents):
        ...
        print(f"Combined cond shape: {cond.shape}")
        pdb.set_trace()
