...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.viewer_zoom = 2.0  # Adjust zoom level for optimal view
        ...
        # Create boundary walls (using Line as in original)
        self.init_boundary(world)
        
        # Create corridor dividing walls (using Box landmarks)
    ...
    def init_boundary(self, world):
        """Create boundary walls using Line landmarks (following navigation_v2 pattern)"""
        top_wall = Landmark(
            name="Top Wall",
            collide=True,
            movable=False,
    ...
    def reset_boundary_walls(self, env_index: int = None):
        """Position boundary walls (following navigation_v2 pattern)"""
