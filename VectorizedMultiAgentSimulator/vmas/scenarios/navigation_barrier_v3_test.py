...
class Scenario(BaseScenario):
    ...
    def init_boundary(self, world):
        ...
        # Central barriers - horizontal and vertical cross pattern
    ...
    def reset_world_at(self, env_index: int = None):
        if self.use_test_positions and self.test_start_positions is not None and self.test_goal_positions is not None:
            ...
            # Use circular sector positioning (original behavior)
            self._set_circular_sector_positions(env_index)
    ...
    def _set_circular_sector_positions(self, env_index: int = None):
        """Original circular sector positioning logic from navigation_barrier_v3.py"""
        ...
        for i, agent in enumerate(self.world.agents):
            # Agent positioned in sector i
            agent_sector = i
            agent_angle = torch.tensor(
                agent_sector * sector_size + sector_size * 0.3, device=device
            )  # 30% offset within sector
            ...
            goal_angle = torch.tensor(
                goal_sector * sector_size + sector_size * 0.7, device=device
            )  # 70% offset within sector
