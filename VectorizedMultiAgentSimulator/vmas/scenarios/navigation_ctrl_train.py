...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        entity_filter_agents: Callable[[Entity], bool] = lambda e: (isinstance(e, Agent) or (isinstance(e, Landmark) and getattr(e, "collide", False)))
        ...
        #         # color=Color.BLACK,
        #         shape=Box(length=0.25, width=0.15),
        #         color=Color.RED,
        #     )
    ...
    def reset_world_at(self, env_index: int = None):
        ScenarioUtils.spawn_entities_randomly(
            ...
            # (0.5, 0.5)
            # (0.5, 0.5)
            (-1.5*self.world_semidim, 1.5*self.world_semidim),
            (-1.5*self.world_semidim, 1.5*self.world_semidim),
        )
        ...
        for i, agent in enumerate(self.world.agents):
            agent_start_pos = agent.state.pos
            
            # Sample random angle and distance
            if env_index is None:
                batch_size = agent_start_pos.shape[0]
                angles = torch.rand(batch_size, device=self.world.device) * 2 * torch.pi
                distances = torch.rand(batch_size, device=self.world.device) * goal_radius
                
                # Convert to cartesian coordinates
                goal_x = agent_start_pos[:, 0] + distances * torch.cos(angles)
                goal_y = agent_start_pos[:, 1] + distances * torch.sin(angles)
                
                # Clamp to world bounds
                goal_x = torch.clamp(goal_x, -1.5*self.world_semidim, 1.5*self.world_semidim)
                goal_y = torch.clamp(goal_y, -1.5*self.world_semidim, 1.5*self.world_semidim)
                
                position = torch.stack([goal_x, goal_y], dim=1).unsqueeze(1)
            else:
                # Single environment case
                angle = torch.rand(1, device=self.world.device) * 2 * torch.pi
                distance = torch.rand(1, device=self.world.device) * goal_radius
                
                goal_x = agent_start_pos[env_index, 0] + distance * torch.cos(angle)
                goal_y = agent_start_pos[env_index, 1] + distance * torch.sin(angle)
                
                # Clamp to world bounds
                goal_x = torch.clamp(goal_x, -1.5*self.world_semidim, 1.5*self.world_semidim)
                goal_y = torch.clamp(goal_y, -1.5*self.world_semidim, 1.5*self.world_semidim)
                
                position = torch.stack([goal_x, goal_y], dim=0).unsqueeze(0).unsqueeze(0)
        ...
        #         env_index=env_index,
        #         world=self.world,
        #         min_dist_between_entities=self.min_distance_between_entities, #Update this. Change based on the circumscribed radius.
        #         x_bounds=(-self.world_semidim, self.world_semidim),
        #         y_bounds=(-self.world_semidim, self.world_semidim),
        #     )
        #     obs_poses.append(position.squeeze(1))
        #     occupied_positions = torch.cat([occupied_positions, position], dim=1)

        # if self.n_obs != 0:
        #     order = torch.randperm(len(self.world.landmarks[self.n_agents : self.n_agents+self.n_obs])).tolist()
        #     obstacles = [self.world.landmarks[self.n_agents : self.n_agents+self.n_obs][i] for i in order]
        
        #     for i,obs in enumerate(obstacles):
        #         # print(i)
        #         # print(obs_poses[i][0])
        #         # print(self.world.landmarks[i+4].name)
        #         obs.set_pos(
        #             torch.tensor(
        #                 [
        #                     obs_poses[i][0][0],
