...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.floor_width = kwargs.pop("width", 3)
        self.floor_length = kwargs.pop("length", 1.5)
        ...
        # Entity filter includes both agents and collidable landmarks
    ...
    def reset_world_at(self, env_index: int = None):
        # Spawn agents in top right quadrant
        ScenarioUtils.spawn_entities_randomly(
            self.world.agents,
            self.world,
            env_index,
            self.min_distance_between_entities,
            (0.0 * self.world_semidim, 1 * self.world_semidim),  # x_bounds: [0, 1]
            (0.0 * self.world_semidim, 1 * self.world_semidim),  # y_bounds: [0, 1]
        )
        ...
        # Position goals in bottom left quadrant
        goal_poses = []
        for _ in self.world.agents:
            position = ScenarioUtils.find_random_pos_for_entity(
                ...
                x_bounds=(-1.5 * self.world_semidim, -0.0 * self.world_semidim),  # x_bounds: [-1.5, 0]
                y_bounds=(-1.5 * self.world_semidim, -0.0 * self.world_semidim),  # y_bounds: [-1.5, 0]
            )
            goal_poses.append(position.squeeze(1))
            occupied_positions = torch.cat([occupied_positions, position], dim=1)

        # Reset agents and assign goals
...
class HeuristicPolicy(BaseHeuristicPolicy):
    def __init__(self, clf_epsilon=0.9, clf_slack=1.0, *args, **kwargs):
    ...
    def compute_action(self, observation: Tensor, u_range: Tensor) -> Tensor:
        ...
        constraints = []
        qp_objective = cp.Minimize(0.1 * cp.sum_squares(u) + self.clf_slack * clf_slack**2)
