...
class Scenario(BaseScenario):
    ...
    def reset_world_at(self, env_index: int = None):
        # Get the device from the world
        device = self.world.device
        
        # Modified agent spawning - spawn agents in top half
        agent_positions = []
        for i, agent in enumerate(self.world.agents):
            occupied_pos_tensor = torch.stack(agent_positions, dim=1) if agent_positions else torch.empty((self.world.batch_dim, 0, 2), device=device)
        ...
        goal_poses = []
        # Properly construct occupied_positions tensor
        occupied_positions = torch.stack(agent_positions, dim=1)  # Shape: [batch_size, num_agents, 2]
        ...
        for i, agent in enumerate(self.world.agents):
            ...
            # Properly concatenate the new position
    ...
    def reset_walls(self, env_index: int = None):
        for landmark in self.world.landmarks:
            if landmark.name == "central_barrier":
