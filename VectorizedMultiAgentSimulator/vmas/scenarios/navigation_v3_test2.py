# Test Scenario for navigation with goal tolerance functionality.
...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        # Goal tolerance parameter - distance threshold for success
        self.goal_tolerance = kwargs.pop("goal_tolerance", 0.05)
        ...
        # Initialize goal-reached tracking
        self.agents_reached_goal = torch.zeros(batch_dim, self.n_agents, dtype=torch.bool, device=device)
    ...
    def reset_world_at(self, env_index: int = 0):
        ...
        if env_index is None:
            self.agents_reached_goal.fill_(False)
        else:
            self.agents_reached_goal[env_index].fill_(False)
    ...
    def _set_random_positions(self, env_index: int = None):
        """Original random positioning code"""
    ...
    def agent_reward(self, agent: Agent):
        ...
        # Check if agent reached goal within tolerance
        agent.on_goal = agent.distance_to_goal < self.goal_tolerance

        # Get agent index
        agent_idx = self.world.agents.index(agent)

        # Update reached goal status
        newly_reached = agent.on_goal & ~self.agents_reached_goal[:, agent_idx]
        self.agents_reached_goal[:, agent_idx] = agent.on_goal

        # Only compute position shaping if agent hasn't reached goal
        pos_shaping = agent.distance_to_goal * self.pos_shaping_factor

        # Zero out reward for agents that have already reached their goal
        agent.pos_rew = torch.where(
            self.agents_reached_goal[:, agent_idx],
            torch.zeros_like(agent.pos_shaping),  # No movement reward if at goal
            agent.pos_shaping - pos_shaping       # Normal shaping reward
        )

        # Add success bonus for newly reached goals
        agent.pos_rew += torch.where(newly_reached, self.final_reward, 0.0)
