...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        # Enhanced entity filter to include collidable landmarks (barriers)
        ...
        # Initialize boundaries and barriers
    ...
    def init_boundary(self, world):
        ...
        # Central barriers - horizontal and vertical
    ...
    def reset_world_at(self, env_index: int = None):
        if self.use_test_positions and self.test_start_positions is not None and self.test_goal_positions is not None:
            ...
            # Use random positioning with barrier constraints
    ...
    def _set_random_positions(self, env_index: int = None):
        """Random positioning with barrier constraints"""
        device = self.world.device

        # Modified agent spawning - spawn agents in top half
        agent_positions = []
        for i, agent in enumerate(self.world.agents):
            occupied_pos_tensor = (
                torch.stack(agent_positions, dim=1) 
                if agent_positions 
                else torch.empty((self.world.batch_dim, 0, 2), device=device)
            )
            
            position = ScenarioUtils.find_random_pos_for_entity(
                occupied_positions=occupied_pos_tensor,
                env_index=env_index,
                world=self.world,
                min_dist_between_entities=self.min_distance_between_entities,
                x_bounds=(-self.floor_length/5, self.floor_length/5),
                y_bounds=(0.1, self.floor_width/2 - 0.1),  # Top half only
            )
            
            agent.set_pos(position.squeeze(1), batch_index=env_index)
            agent_positions.append(position.squeeze(1))

        # Modified goal spawning - spawn goals in bottom half
        goal_poses = []
        occupied_positions = torch.stack(agent_positions, dim=1)
        ...
        for i, agent in enumerate(self.world.agents):
            position = ScenarioUtils.find_random_pos_for_entity(
                ...
                world=self.world,
                min_dist_between_entities=self.min_distance_between_entities,
                x_bounds=(-self.floor_length/2, self.floor_length/2),
                y_bounds=(-self.floor_width/2 + 0.1, -0.1),  # Bottom half only
            )
            
            goal_poses.append(position.squeeze(1))
            occupied_positions = torch.cat([occupied_positions, position], dim=1)

        # Reset agent goals based on sharing configuration
        for i, agent in enumerate(self.world.agents):
            if self.split_goals:
    ...
    def reset_walls(self, env_index: int = None):
        for landmark in self.world.landmarks:
            if landmark.name == "Central Barrier H":
                # Position horizontal barrier at center
                landmark.set_pos(
                    torch.tensor(
                ...
                # Position vertical barrier at center
                landmark.set_pos(
                    torch.tensor(
                        [0.0, 0.0],  # Center of environment
                        dtype=torch.float32,
                ...
                landmark.set_rot(
                    torch.tensor(
                        [torch.pi / 2],  # Vertical orientation
                        dtype=torch.float32,
                        device=self.world.device,
                    ),
                    batch_index=env_index,
