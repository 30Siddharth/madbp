...
class NormalizedTrajectoryDataset(Dataset):
    ...
    def _compute_xy_normalization_stats(self):
        ...
        for ep in self.episodes[:min(100, len(self.episodes))]:
            traj = torch.tensor(self.h5['trajectories'][ep][:])
    ...
    def __getitem__(self, idx):
        ...
        # print("Original goals:", goal_xy)
        # print("Normalized goals:", goal_xy_norm)  
...
def process_receding_horizon_training_simplified(model, optimizer, frames, starts, goals, n_agents, full_trajs,
    ...
    for horizon_idx in range(num_horizons):
        ...
        # Forward pass through model - FIXED: Pass n_agents consistently
...
def train_with_receding_horizon():
    ...
    epochs = max_epochs
    horizon_size = full_horizon-1  # Fixed horizon size for receding horizon
    ...
    writer = SummaryWriter(f"runs/temporal_madp_{scenario_name}_horizon_loss_cm_axpro")
    os.makedirs("samples_moving_horizon", exist_ok=True)
    ...
    for epoch in tqdm(range(epochs), desc="Boundary-Constrained Receding Horizon Training"):
        ...
        if epoch % 100 == 0 or epoch == epochs - 1:
            print(f"Epoch {epoch}: Loss = {avg_loss:.6f}, Active Agents = {current_agents}, Progress = {progress:.3f}")
            ...
            with torch.no_grad():
                ...
                for i in range(min(na.item(), 4)):
                    ...
                    ax.plot(pr[:, 0], pr[:, 1], '-o', color=color,
                           label=f'Pred Agent {i}', alpha=0.8, linewidth=2)
                    ...
                    ax.scatter(gt[0, 0], gt[0, 1], c='green', s=100, marker='v',
                              edgecolors='black', linewidth=2)
                    ax.scatter(pr[-1, 0], pr[-1, 1], c='red', s=100, marker='^',
                              edgecolors='black', linewidth=2)
                ...
                plt.tight_layout()
                plt.savefig(f"samples_moving_horizon/boundary_constrained_epoch_{epoch}.png", dpi=150, bbox_inches='tight')
                plt.close()

    # Save final model
    writer.close()
    torch.save(model.state_dict(), f"boundary_constrained_madp_{scenario_name}_{n_agents}.pth")
