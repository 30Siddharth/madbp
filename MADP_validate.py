...
class HierarchicalMovingHorizonController:
    """
    Complete hierarchical moving horizon controller implementation that combines:
    1. MADP diffusion model for trajectory planning
    2. HeuristicPolicy CLF-QP controller from scenario file
    3. Hierarchical control strategy with distance-based switching
    4. Moving horizon approach for real-time execution
    """
    ...
    def __init__(self, scenario_name=None, num_envs=1, max_steps=40, n_agents=4, 
        ...
        Args:
            ...
            goal_pursuit_threshold: Distance threshold to switch to goal pursuit (0.25)
        """
        self.device = device
        ...
        self.env = make_env(
            # scenario=f"{scenario_name}_test",
    ...
    def compute_hierarchical_control(self, target_positions, target_velocities, goal_positions):
        """
        Compute hierarchical control actions using HeuristicPolicy CLF-QP with proper device handling
        
        This function implements a three-tier hierarchical control strategy:
        1. Agents within goal_tolerance (0.1m): Stop and mark as successful
        2. Agents within goal_pursuit_threshold (0.25m): Use CLF-QP for direct goal pursuit
        3. Other agents: Use CLF-QP for trajectory tracking with modified observations
        
        Key fixes implemented:
        - CPU tensor conversion for CLF-QP compatibility
        - Proper device management throughout
        - Enhanced error handling with detailed fallbacks
        - Correct observation modification for trajectory tracking
        - Goal position consistency verification
        
        Args:
            target_positions: MADP target positions [n_agents, 2]
            target_velocities: MADP target velocities [n_agents, 2] 
            goal_positions: Final goal positions [n_agents, 2]
        
        Returns:
            Control actions [n_agents, 2]
        """
        ...
        # Control range for VMAS (ensure it's a tensor)
        ...
        # Debug: Verify goal position consistency
        ...
        if self._debug_step_count % 20 == 0:
            self._debug_goal_consistency(current_positions, goal_positions)
        ...
        for i in range(self.n_agents):
            try:
                if self.agents_stopped[i]:
                    ...
                    # Agent is close to goal - use CLF-QP for direct goal pursuit
                    ...
                    # Agent follows MADP trajectory - use CLF-QP for trajectory tracking
                    current_modes.append('trajectory_tracking')
                    
                    success = self._compute_trajectory_tracking_action(
                        i, observations, u_range, actions, current_positions,
                        current_velocities, target_positions, target_velocities
                    )
    ...
    def _compute_goal_pursuit_action(self, agent_idx, observations, u_range, actions, 
        ...
        try:
            ...
            # Fallback to proportional control for goal pursuit
    ...
    def _compute_trajectory_tracking_action(self, agent_idx, observations, u_range, actions,
                                        current_positions, current_velocities, 
                                        target_positions, target_velocities):
        """
        Compute CLF-QP action for trajectory tracking with modified observations
        
        Returns:
            bool: True if CLF-QP succeeded, False if fallback was used
        """
        try:
            # Create modified observation for trajectory tracking
            modified_obs = observations[agent_idx].clone().detach()
            
            # Replace goal relative position with target relative position
            # Observation format: [agent_pos(2), agent_vel(2), goal_relative_pos(2), lidar_data(...)]
            current_pos = current_positions[agent_idx]
            target_relative_pos = current_pos - target_positions[agent_idx]
            
            # Verify observation structure and update goal relative position
            if modified_obs.shape[0] >= 6:
                modified_obs[4:6] = target_relative_pos
            else:
                # If observation format is different, create a new one
                modified_obs = torch.cat([
                    current_pos,  # agent position
                    current_velocities[agent_idx],  # agent velocity  
                    target_relative_pos,  # target relative position (replacing goal)
                    modified_obs[6:] if modified_obs.shape[0] > 6 else torch.tensor([])
                ])
            
            # Convert to CPU for CLF-QP
            obs_cpu = modified_obs.detach().cpu()
            u_range_cpu = u_range.detach().cpu()
            ...
            # Fallback to PD control for trajectory tracking
            pos_error = target_positions[agent_idx] - current_positions[agent_idx]
            vel_error = target_velocities[agent_idx] - current_velocities[agent_idx]
    ...
    def _debug_goal_consistency(self, current_positions, goal_positions):
        """
        Debug function to verify goal position consistency between environment and dataset
        """
        try:
            for i, agent in enumerate(self.env.agents):
                if hasattr(agent, 'goal') and hasattr(agent.goal, 'state'):
                    env_goal = agent.goal.state.pos[0]  # Goal from environment
                    dataset_goal = goal_positions[i]     # Goal from dataset
                    
                    distance_diff = torch.norm(env_goal - dataset_goal)
                    
                    if distance_diff > 0.01:  # Threshold for significant difference
                        print(f"WARNING: Goal mismatch for agent {i}:")
                        print(f"  Environment goal: {env_goal.cpu().numpy()}")
                        print(f"  Dataset goal: {dataset_goal.cpu().numpy()}")
                        print(f"  Difference: {distance_diff.item():.4f}")
        except Exception as e:
            pass  # Suppress debug errors
    ...
    def execute_hierarchical_trajectory(self, model, frames, starts, goals, n_agents, 
        ...
        2. Hierarchical control mode switching
        3. CLF-QP control for both tracking and goal pursuit
        ...
        for step in range(self.max_steps):
            ...
            # Compute hierarchical control actions
    ...
    def evaluate_performance(self, results):
        ...
        mode_counts = {'trajectory_tracking': 0, 'goal_pursuit': 0, 'stopped': 0}
    ...
    def plot_comprehensive_analysis(self, results, plot_name="hierarchical_analysis.png"):
        ...
        plt.title('Hierarchical Trajectory Comparison')
        ...
        plt.subplot(2, 3, 2)
        control_modes = results['control_modes']
        mode_colors = {'trajectory_tracking': 'blue', 'goal_pursuit': 'orange', 'stopped': 'red'}
        ...
        plt.title('Control Mode Timeline')
        ...
        plt.axhline(y=self.goal_pursuit_threshold, color='orange', linestyle='--', 
                   label=f'Goal Pursuit Threshold: {self.goal_pursuit_threshold}')
...
def execute_hierarchical_moving_horizon_system(scenario_name, model_path=None, device='cuda',
    ...
    1. Loads the trained MADP diffusion model
    2. Initializes the hierarchical controller with HeuristicPolicy CLF-QP
    3. Executes the hierarchical moving horizon strategy
    ...
    Args:
        ...
        goal_pursuit_threshold: Goal pursuit mode threshold (0.25)
    ...
    try:
        ...
        # h5_path = "navigation_v2_Na_4_T_50_dataset.h5"
    ...
    print(f"Goal Pursuit Threshold: {goal_pursuit_threshold}")
    ...
    model = EnhancedMultiAgentDiffusionModel(
        max_agents=10,
        horizon=horizon_size,  # Use horizon_size for moving horizon
        state_dim=2,
        img_ch=3,
        hid=128,
    ...
    controller = HierarchicalMovingHorizonController(
        ...
        num_envs=1,
        max_steps=full_horizon + 20,  # Extra steps for completion
        n_agents=num_agents,
        device=device,
    ...
    print("\nExecuting hierarchical moving horizon trajectory...")
    ...
    gif_name = f"hierarchical_madp_{scenario_name}.gif"
    controller.create_gif_from_frames(results, gif_name)
    
    plot_name = f"hierarchical_analysis_{scenario_name}.png"
    ...
    print("\n" + "="*70)
    print("HIERARCHICAL MOVING HORIZON SYSTEM - EXECUTION RESULTS")
    print("="*70)
...
if __name__ == "__main__":
    ...
    # Execute the complete hierarchical system
