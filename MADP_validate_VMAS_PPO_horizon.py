...
from PPO_controller_validation import SimplePPOController

class MovingHorizonTrajectoryExecutor:
    def __init__(self, scenario_name=None, num_envs=1, max_steps=40, n_agents=4, device='cuda',
                 test_start_positions=None, test_goal_positions=None, horizon_size=8, ppo_controller_path=None):
        """
        ...
        if ppo_controller_path:
            try:
                self.ppo_controller = SimplePPOController(ppo_controller_path, device)
                self.use_ppo = True
                print(f"Using PPO controller: {ppo_controller_path}")
            except Exception as e:
                print(f"Failed to load PPO controller: {e}")
    ...
    def smooth_trajectory_segment(self, trajectory_segment, method='cubic_spline', strength=0.2):
    ...
    def get_agent_actions(self, target_positions=None, target_velocities=None):
        """Get actions using PPO controller or PD controller"""
        if self.use_ppo:
            # Use PPO controller - FIXED to match PD controller format
            ppo_actions = self.ppo_controller.get_actions_for_all_agents(self.env)
            return ppo_actions
        else:
            # Use existing PD controller
            forces = self.compute_control_forces(None, target_positions, target_velocities)
            
            # Create actions in the same format as PPO controller
    ...
    def execute_moving_horizon_trajectory(self, model, frames, starts, goals, n_agents, 
        ...
        for horizon_idx in range(num_horizons):
            ...
            for t in range(min(current_horizon_length - 1, self.max_steps - len(executed_positions) - 1)):
                ...
                ''' Actions with learned controller'''
                actions = self.get_agent_actions(target_pos, target_vel)
    ...
    def evaluate_moving_horizon_performance(self, results):
        ...
        metrics = {
            ...
            'success_rate': 1.0 if final_position_error < 0.15 else 0.0,  # Slightly relaxed for moving horizon
            'avg_generation_time': avg_generation_time,
            'total_horizons': total_horizons,
            'real_time_capable': avg_generation_time < 0.1  # 100ms threshold for real-time
        }
...
def execute_moving_horizon_diffusion_trajectory_in_vmas(scenario_name, model_path=None, device='cuda'):
    ...
    # ppo_controller_path = "None"
    # ppo_controller_path = "simple_ppo_controller.pth"
    ppo_controller_path = "vectorized_ppo_controller.pth"
